...
public class ExceptionResolverConfiguration {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ExceptionResolverConfiguration.class);
  ...
  private class SystemErrorAttributes extends DefaultErrorAttributes {
    ...
    public Map<String, Object> getErrorAttributes(RequestAttributes requestAttributes,
      ...
      errorAttributes.put("timestamp", CommonDateUtils.date2String(new Date(),
              CommonDateUtils.DatePattern.yyyyMMddHHmmssLine));// 错误发生的时间戳
      addStatus(errorAttributes, requestAttributes);// 错误发生状态
      // 添加错误的详细信息，包含异常类、用户自定义的异常信息
      addErrorDetails(errorAttributes, requestAttributes, includeStackTrace);
      // 添加URL请求的地址
      errorAttributes.put("data", "");
      LOGGER.debug("ExceptionResolverConfiguration date={}", errorAttributes.get("timestamp"));

      HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
      String requestId = request.getParameter("requestId");
      errorAttributes.put("requestId", requestId);
      errorAttributes.put("responseId", "");
    ...
    private void addErrorDetails(Map<String, Object> errorAttributes,
      ...
      if (error != null) {
        ...
        errorAttributes.put("message", UserException.MAINTAINING);
      }
