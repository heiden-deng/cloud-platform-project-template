...
public abstract class AbsController {
   ...
   * 
   * @param bindingResult 绑定的错误对象<br>
  ...
  protected ModelAndView createModelAndView(RequestResult requestResult) {
    ...
    if (requestResult.getMessage() != null) {
      modelMap.put("message", requestResult.getMessage());
    }
    if (requestResult.getRequestId() != null) {
      modelMap.put("requestId", requestResult.getRequestId());
    }

    if (requestResult.getRequestId() != null) {
      modelMap.put("responseId", requestResult.getResponseId());
    }

    if (requestResult.getTimestamp() != null) {
      modelMap.put("timestamp", requestResult.getTimestamp());
    }
   ...
   *
   * @param bindingResult
   * @param bindingResultHolder 绑定信息验证结果持有器
   * 
   * @return boolean true：信息验证有误；false:信息验证无误
   *
   */
  protected boolean validateBindingError(BindingResult bindingResult,
      BindingResultHolder bindingResultHolder) {
    Map<String, List<String>> errorMsg = parseErrorMsg(bindingResult);
    RequestResult result = null;
    if (errorMsg != null) {
      result = new RequestResult();
      // 应天海特殊要求自出暂时关闭
      // result.setMessage(errorMsg.s);
